...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* ══ GITHUB PAGES 방어 : Jekyll 테마 래퍼가 폭·폰트를 눌러도 무시 ══
     테마 CSS가 .container/.markdown-body 등으로 max-width를 걸어도
     풀블리드 섹션은 항상 뷰포트 전체를 차지하게 강제한다. */
  ...
  /* 히어로 내부는 1240px 래퍼를 쓰지 않는다 — 글자가 화면 끝까지 가야 하므로 */
  ...
  .mega{
    ...
    /* vw 고정값이 아니라 '여백을 뺀 실제 폭 ÷ 글자수 비율'로 계산 →
       어떤 뷰포트에서도 좌우 끝까지 꽉 차게 떨어진다 */
  ...
  /* DREAM(5자)을 WALLST(6자) 폭에 맞춰 늘려 두 줄이 같은 폭으로 정렬 */
